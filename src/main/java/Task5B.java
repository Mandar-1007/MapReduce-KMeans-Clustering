...
public class Task5B {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String outputBasePath = ".../Project2/output/task5b";
        ...
        // Load data points and print their assignments
        System.out.println();
        System.out.println("Final clustered data points:");
        FileSystem fs = FileSystem.get(conf);
        try (BufferedReader br = new BufferedReader(new InputStreamReader(fs.open(new Path(inputPath))))) {
            String line;
            while ((line = br.readLine()) != null) {
                ...
                // Find nearest centroid for final assignments
                for (int j = 0; j < finalCentroids.size(); j++) {
                    double[] centroid = finalCentroids.get(j);
                    double distance = Math.sqrt(Math.pow(x - centroid[0], 2)
                            + Math.pow(y - centroid[1], 2)
                            + Math.pow(z - centroid[2], 2));
                ...
                // Output the point with its nearest centroid
                System.out.println("Point: " + x + "," + y + "," + z + " -> Assigned to Cluster " + nearestCentroid +
                        " (Centroid: " + finalCentroids.get(nearestCentroid)[0] + "," + finalCentroids.get(nearestCentroid)[1] +
                        "," + finalCentroids.get(nearestCentroid)[2] + ")");
            }
        }
    }
}
